...
void onInit( CBlob@ this )
{
	...
	this.set_f32("powerFactor", 0.04f);
	this.set_u32( "onTime", 0 );

	CSprite@ sprite = this.getSprite();
    CSpriteLayer@ thruster = sprite.addSpriteLayer( "thruster" );
    if (thruster !is null)
    {
    	thruster.SetOffset(Vec2f(0,8));
    	thruster.SetRelativeZ(2);
    	thruster.SetLighting( false );

        Animation@ animcharge = thruster.addAnimation( "go", 2, true );
        animcharge.AddFrame(32);
        animcharge.AddFrame(33);
        animcharge.AddFrame(34);
        thruster.SetAnimation("go");
        thruster.SetVisible(false);
    }

    sprite.SetEmitSound("IntThrust");
...
void onTick( CBlob@ this )
{
	...
	CSpriteLayer@ thruster = sprite.getSpriteLayer("thruster");
	if ( thruster !is null )
	{
		thruster.animation.time = on ? 1 : 0;
    	thruster.SetVisible(on);
	}
...
{
	//if ( customData == 9 )
	//	directionalSoundPlay( "thrusterHit.ogg", worldPoint );		
}
